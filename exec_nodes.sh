...
if [ ${#nodes[@]} -eq 0 ]; then
    echo "Warning: the nodes ${NODE} is empty, please input a valid one."
    exit 0
fi
...
for node in ${nodes[@]}
do
    if [[ "x${node}" != "xfirstbox" ]] && [ ! -f /pitrix/conf/settings/${node} ]; then
...
log_file="/pitrix/log/exec_nodes/exec_nodes.log"
if [ -f ${log_file} ]; then
    echo "" >> ${log_file}
fi
...
cat ${conf_file} | parallel -j 20 --colsep '#' --joblog ${job_log} ${CWD}/_exec_node.sh {1} {2} 2>&1 | tee -a ${log_file}
